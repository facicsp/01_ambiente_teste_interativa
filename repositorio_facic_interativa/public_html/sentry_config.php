...
if (!file_exists(__DIR__ . '/vendor/autoload.php')) {
    // Se não existir, não faz nada (sistema continua funcionando sem Sentry)
    return;
}
...
try {
    ...
    // Se houver erro ao inicializar Sentry, apenas ignora
    // O sistema continua funcionando normalmente
    error_log("Erro ao inicializar Sentry: " . $e->getMessage());
    return;
}
    ...
    try {
        \Sentry\configureScope(function (\Sentry\State\Scope $scope) use ($contextoAcademico) {
            ...
            foreach ($contextoAcademico as $key => $value) {
                $scope->setContext($key, is_array($value) ? $value : ['valor' => $value]);
            }
        ...
        // Silenciosamente falha se Sentry não estiver disponível
        error_log("Erro ao capturar exceção no Sentry: " . $e->getMessage());
    }
}
...
function capturarQueryLentaFACIC(string $query, float $tempo, array $parametros = []) {
    try {
        ...
        error_log("Erro ao capturar query lenta no Sentry: " . $e->getMessage());
    }
}
...
function capturarSistemaForaDoArFACIC(\Throwable $exception, array $dadosInfraestrutura = []) {
    try {
        ...
        error_log("Erro ao capturar sistema fora do ar no Sentry: " . $e->getMessage());
    }
}
...
function capturarFalhaSegurancaFACIC(\Throwable $exception, array $dadosSeguranca = []) {
    try {
        ...
        error_log("Erro ao capturar falha de segurança no Sentry: " . $e->getMessage());
    }
}
...
function capturarProblemaRefatoracaoFACIC(string $modulo, string $problema, array $contexto = []) {
    try {
        ...
        error_log("Erro ao capturar problema de refatoração no Sentry: " . $e->getMessage());
    }
}
...
function capturarMensagemSentry(string $message, string $level = 'info', array $context = []) {
    try {
        ...
        $severityMethod = $level;
        \Sentry\captureMessage($message, \Sentry\Severity::$severityMethod());
    } catch (\Exception $e) {
        error_log("Erro ao capturar mensagem no Sentry: " . $e->getMessage());
    }
}
...
set_exception_handler(function (\Throwable $exception) {
    try {
        ...
        error_log("Erro no exception handler do Sentry: " . $e->getMessage());
    }
});
...
register_shutdown_function(function () {
    try {
        ...
        error_log("Erro no shutdown handler do Sentry: " . $e->getMessage());
    }
});

// REMOVIDO: Log de inicialização automática
// Isso causava problemas ao carregar em cada página
// A inicialização será registrada apenas em testes específicos
